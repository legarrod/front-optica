							...
							data?.map((row, index) => (
								<StyledTableRow key={index}>
									<StyledTableCell align="left">
										<p className=""> {`${row.nombre} ${row.apellidos}`}</p>
									</StyledTableCell>
									...
									<StyledTableCell align="center">
										<select
											className=" p-2 w-28 text-center rounded-lg my-3"
											name="fk_id_estado"
											onChange={(e) => handlerSaveHour({ row, e })}
										>
									...
									<StyledTableCell align="center">
										<Link href={`/citas/?${row.id_cita_paciente}`}>
											<a>
