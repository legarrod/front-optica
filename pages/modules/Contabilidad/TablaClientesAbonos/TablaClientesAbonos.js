...
export default function TablaClientesAbonos({
  ...
  const getAbonos = async (
    urlObtenerAbonosFactura,
    callbackResponseGetAbonos = null
  ) => {
    try {
  ...
  const verListadoAbonos = (value) => {
    ...
    getAbonos(
      `${urlObtenerAbonosFactura}${value.id}`,
      callbackResponseGetAbonos
    );
  ...
  const handlerSaveState = (data) => {
    ...
    let newData = {
      estado: data.e.target.options[index].text,
      id_factura: parseInt(data.row.id),
    };
  ...
  const formatData = (rowTemporal) => {
    if (controlRefresFactura) {
      allInvoices.filter((item) => {
        item.id === rowTemporal.id && setDataInfo(item);
      });
  ...
  return (
    <div className="flex flex-col flex-wrap lg:flex-nowrap md:flex-row w-full">
                ...
                {allInvoices?.map((row, index) => (
                  <StyledTableRow key={index}>
                    <StyledTableCell align="center">
                      <button>
                        <RemoveRedEyeIcon
                          onClick={() => verListadoAbonos(row)}
                        />
                      </button>
                      <button>
                        <CloudDownloadIcon
                          onClick={() => handlerBuscarFactura(row)}
                        />
                      </button>
                      <button>
                        <DeleteIcon onClick={() => eliminarFactura(row)} />
                      </button>
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {row.cedula}
                    ...
                    <select
                      className={
                        row.estado === 'Pendiente'
                          ? 'bg-yellow-400 p-2 w-28 text-center rounded-lg my-3'
                          : row.estado === 'Pagada'
                          ? 'bg-green-400 p-2 w-28 text-center rounded-lg my-3'
                          : 'bg-red-400 p-2 w-28 text-center rounded-lg my-3'
                      }
                      name="fk_id_estado"
                      ref={register}
                      onChange={(e) => handlerSaveState({ row, e })}
                    >
        ...
        <div className="mx-0 flex flex-col w-full md:w-4/5 mt-10 md:mt-0 border-2 p-2 rounded-lg mb-5 ticket">
          <div className="flex flex-row-reverse w-full justify-between none-print">
            {parseInt(dataInfo?.valor_factura) >
              parseInt(
                dataInfo?.total_deuda === null ? 0 : dataInfo?.total_deuda
              ) &&
              dataInfo?.estado !== 'Pagada' && (
            ...
            <div className="flex flex-wrap justify-between css-print">
              <div>
                ...
                <div className="flex flex-row">
                  ...
                  <p className="text-xs ml-3 mt-1 text-gray-700">
                    $ {dataInfo?.total_deuda ? dataInfo?.total_deuda : '---'}
                  </p>
                </div>
              </div>
              <div>
      ...
      {viewFactura && (
        <Factura
          facturaPersona={facturaPersona && facturaPersona[0]}
      ...
      >
        <Fade in={open}>
          <div className={`${classes.paper} mx-2 md:mx-20 flex flex-col px-20`}>
            ...
            <div className="">
              ...
              >
                ...
                />
                <textarea
                  className="border-b-4 border-2 w-full border-gray-400 rounded-md mb-3 px-2 h-20 shadow-lg"
                  name="note"
                  placeholder="Nota"
